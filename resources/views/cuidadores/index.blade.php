            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p class="text-slate-500 text-sm mt-1">
                        Administración de cuidadores registrados en el sistema
                    </p>
                </div>
            </div>

            <!-- TOOLBAR: BUSQUEDA, FILTRO, ORDENAR Y AGREGAR CUIDADOR -->
            <div class="bg-white border border-slate-200/80 rounded-2xl p-4 shadow-sm">
                <div class="flex flex-col md:flex-row items-center gap-3">
                    
                    <!-- Campo Buscar cuidador -->
                    <form action="{{ route('cuidadores.index') }}" method="GET" class="relative flex-1 w-full">
                        ...
                        <input 
                            ...
                            name="buscar"
                            value="{{ $search ?? '' }}"
                            placeholder="Buscar cuidador por nombre, correo, usuario o teléfono..." 
                    ...
                    <a href="{{ route('cuidadores.create') }}" class="button-blue">
            ...
            <div class="bg-white border border-slate-200/80 rounded-3xl overflow-hidden shadow-sm">
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-slate-50/80 border-b border-slate-200 text-slate-700 text-xs font-bold uppercase tracking-wider">
                                ...
                                <th class="py-4 px-6 whitespace-nowrap">Apellidos</th>
                                <th class="py-4 px-6 whitespace-nowrap">Correo</th>
                                <th class="py-4 px-6 whitespace-nowrap">Teléfono</th>
                                <th class="py-4 px-6 whitespace-nowrap">Usuario</th>
                        ...
                        <tbody class="divide-y divide-slate-100 text-sm">
                            @forelse($cuidadores as $cuidador)
                                @php
                                    $initials = strtoupper(substr($cuidador->nombre ?? 'C', 0, 1) . substr($cuidador->apellido_paterno ?? 'A', 0, 1));
                                    $isActive = in_array(strtolower($cuidador->estado ?? ''), ['active', 'activo']);
                                @endphp
                                <tr class="hover:bg-slate-50/60 transition duration-150">
                                    <td class="py-4 px-6 font-bold text-slate-800 whitespace-nowrap">
                                        <div class="flex items-center space-x-3">
                                            ...
                                            <span>{{ $cuidador->nombre }}</span>
                                        </div>
                                    </td>
                                    <td class="py-4 px-6 text-slate-600 font-medium whitespace-nowrap">
                                        {{ $cuidador->apellido_paterno }} {{ $cuidador->apellido_materno }}
                                    </td>
                                    <td class="py-4 px-6 text-slate-600 font-medium whitespace-nowrap">
                                        {{ $cuidador->correo ?? '-' }}
                                    </td>
                                    ...
                                    <td class="py-4 px-6 text-slate-600 font-medium whitespace-nowrap">
                                        <span class="px-2.5 py-1 bg-slate-100 text-slate-700 rounded-lg text-xs font-mono">
                                            {{ $cuidador->usuario }}
                                        </span>
                                    </td>
                                    <td class="py-4 px-6 whitespace-nowrap">
                                    ...
                                    <td class="py-4 px-6 text-right whitespace-nowrap">
                                        <a href="{{ route('cuidadores.detalle_cuidador', ['id' => $cuidador->id]) }}" class="inline-block px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-[#0C3B5E] font-semibold text-xs rounded-lg transition duration-150 whitespace-nowrap">
                                            Ver / Editar
                                        </a>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="7" class="py-8 px-6 text-center text-slate-400 font-medium">
                                        No se encontraron cuidadores registrados en el sistema (Rol #2).
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
                ...
                <div class="bg-slate-50/80 px-6 py-4 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-4">
                    <p class="text-xs font-medium text-slate-500">
                        Mostrando <span class="font-bold text-slate-700">{{ $cuidadores->firstItem() ?? 0 }}</span> a <span class="font-bold text-slate-700">{{ $cuidadores->lastItem() ?? 0 }}</span> de <span class="font-bold text-slate-700">{{ $cuidadores->total() }}</span> cuidadores
                    </p>
                    <div>
                        {{ $cuidadores->links() }}
                    </div>
