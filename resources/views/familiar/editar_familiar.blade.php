    ...
    <div class="min-h-screen bg-slate-50 flex flex-col md:flex-row">
        ...
        <main class="flex-1 p-6 md:p-10 space-y-6">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 class="text-2xl md:text-3xl font-extrabold text-[#0C3B5E] tracking-tight">
                        Editar Familiar: {{ $familiar->nombre }} {{ $familiar->apellido_paterno }}
                    </h1>
            ...
            @if (session('success'))
                <div x-data="{ open: true }" x-show="open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-xs">
                    <div class="bg-white rounded-3xl p-6 md:p-8 max-w-md w-full shadow-2xl border border-slate-100 text-center space-y-5 transform transition-all">
                        ...
                        <div>
                            <h3 class="text-xl font-extrabold text-[#0C3B5E]">¡Familiar Editado Correctamente!</h3>
                        ...
                        <div class="pt-2 flex flex-col sm:flex-row gap-3">
                            ...
                            <a href="{{ route('familiar.detalle_familiar', ['id' => $familiar->id]) }}" class="ver-detalles">
                                Ver detalles
                            </a>
                        </div>
                    </div>
            ...
            @if ($errors->any())
                <div class="p-4 bg-rose-50 border border-rose-200 text-rose-700 rounded-2xl text-sm shadow-sm">
                    <div class="flex items-center gap-2 mb-1">
                        ...
                        <p class="font-bold">Hubo un problema al actualizar el familiar:</p>
                    </div>
            ...
            <div class="bg-white border border-slate-200/80 rounded-3xl overflow-hidden shadow-sm">
                ...
                <form action="{{ route('familiar.update', ['id' => $familiar->id]) }}" method="POST" class="p-6 md:p-8 space-y-8"
                      x-data="{
                        internos: [
                            @if(isset($familiar->familyLinks) && $familiar->familyLinks->count() > 0)
                                @foreach ($familiar->familyLinks as $link)
                                    @php $res = $link->resident; @endphp
                                    @if($res)
                                        { id: {{ $res->id }}, nombre: '{{ addslashes($res->nombre . ' ' . $res->apellido_paterno) }}', parentesco: '{{ addslashes($link->parentesco ?? '') }}' },
                                    @endif
                                @endforeach
                            @elseif(isset($familiar->internos))
                        ...
                        <div class="space-y-5">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                <div>
                                    ...
                                    <input type="text" name="nombre" value="{{ old('nombre', $familiar->nombre) }}" required class="w-full rounded-xl border-slate-300 shadow-sm focus:border-[#4EBA87] focus:ring focus:ring-[#4EBA87]/20 transition duration-200 text-sm @error('nombre') border-rose-500 @enderror">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="apellido_paterno" value="{{ old('apellido_paterno', $familiar->apellido_paterno) }}" required class="w-full rounded-xl border-slate-300 shadow-sm focus:border-[#4EBA87] focus:ring focus:ring-[#4EBA87]/20 transition duration-200 text-sm @error('apellido_paterno') border-rose-500 @enderror">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                <div>
                                    ...
                                    <input type="text" name="apellido_materno" value="{{ old('apellido_materno', $familiar->apellido_materno) }}" required class="w-full rounded-xl border-slate-300 shadow-sm focus:border-[#4EBA87] focus:ring focus:ring-[#4EBA87]/20 transition duration-200 text-sm @error('apellido_materno') border-rose-500 @enderror">
                                ...
                                <div>
                                    ...
                                    <input type="email" name="correo" value="{{ old('correo', $familiar->correo) }}" required class="w-full rounded-xl border-slate-300 shadow-sm focus:border-[#4EBA87] focus:ring focus:ring-[#4EBA87]/20 transition duration-200 text-sm @error('correo') border-rose-500 @enderror">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                <div>
                                    ...
                                    <input type="text" name="telefono" value="{{ old('telefono', $familiar->telefono) }}" required class="w-full rounded-xl border-slate-300 shadow-sm focus:border-[#4EBA87] focus:ring focus:ring-[#4EBA87]/20 transition duration-200 text-sm @error('telefono') border-rose-500 @enderror">
                        ...
                        <div class="space-y-5">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                <div>
                                    ...
                                    <input type="text" name="usuario" value="{{ old('usuario', $familiar->usuario) }}" required class="w-full rounded-xl border-slate-300 shadow-sm focus:border-[#4EBA87] focus:ring focus:ring-[#4EBA87]/20 transition duration-200 text-sm @error('usuario') border-rose-500 @enderror">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                ...
                                <div>
                                    ...
                                    <select name="estado" class="w-full rounded-xl border-slate-300 shadow-sm focus:border-[#4EBA87] focus:ring focus:ring-[#4EBA87]/20 transition duration-200 text-sm text-slate-600">
                                        <option value="Activo" {{ old('estado', $familiar->estado) == 'active' || old('estado', $familiar->estado) == 'Activo' ? 'selected' : '' }}>Activo</option>
                                        <option value="Inactivo" {{ old('estado', $familiar->estado) == 'inactive' || old('estado', $familiar->estado) == 'Inactivo' ? 'selected' : '' }}>Inactivo</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                    ...
                    <div>
                        ...
                        <div class="overflow-x-auto rounded-xl border border-slate-200">
                            <table class="w-full text-sm text-left">
                                <thead class="bg-slate-50 text-slate-500 uppercase text-xs font-semibold tracking-wider">
                                    <tr>
                                        <th class="px-4 py-3">Nombre del interno</th>
                                ...
                                <tbody class="divide-y divide-slate-100">
                                    <template x-for="(fila, index) in internos" :key="fila.id">
                                        <tr class="hover:bg-slate-50 transition duration-150">
                                            ...
                                            <td class="px-4 py-3">
                                                <select :name="'internos[' + index + '][parentesco]'" x-model="fila.parentesco" class="w-full rounded-lg border-slate-300 shadow-sm focus:border-[#4EBA87] focus:ring focus:ring-[#4EBA87]/20 transition duration-200 text-sm text-slate-600">
                                            ...
                                            <td class="px-4 py-3 text-right">
                                                <button type="button" @click="internos.splice(index, 1)" class="text-rose-600 hover:underline font-semibold text-xs">
                                                    Eliminar
                                                </button>
                                            </td>
                                        </tr>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mt-4">
                            <div>
                                <label class="block text-xs font-semibold text-slate-500 mb-1">Seleccionar interno</label>
                                <select x-model="nuevoInternoId" class="w-full rounded-xl border-slate-300 shadow-sm focus:border-[#4EBA87] focus:ring focus:ring-[#4EBA87]/20 transition duration-200 text-sm text-slate-600">
                                    ...
                                    @if(isset($internosDisponibles))
                                        @foreach ($internosDisponibles as $interno)
                                            <option value="{{ $interno->id }}" data-nombre="{{ $interno->nombre }} {{ $interno->apellido_paterno }}">{{ $interno->nombre }} {{ $interno->apellido_paterno }} {{ $interno->apellido_materno }}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
                            <div>
                                <label class="block text-xs font-semibold text-slate-500 mb-1">Parentesco</label>
                                <select x-model="nuevoParentesco" class="w-full rounded-xl border-slate-300 shadow-sm focus:border-[#4EBA87] focus:ring focus:ring-[#4EBA87]/20 transition duration-200 text-sm text-slate-600">
                                    <option value="">Selecciona parentesco</option>
                                    <option value="Hijo(a)">Hijo(a)</option>
                                    <option value="Esposo(a)">Esposo(a)</option>
                                    <option value="Hermano(a)">Hermano(a)</option>
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            <div>
                                ...
                                <input type="text" readonly value="{{ in_array(strtolower($familiar->estado), ['active', 'activo']) ? 'Activo' : 'Inactivo' }}" class="w-full rounded-xl border-slate-200 bg-slate-50 text-slate-500 shadow-sm focus:ring-0 cursor-not-allowed text-sm">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-end gap-4 pt-6 border-t border-slate-200 mt-8">
                        <a href="{{ route('familiar.detalle_familiar', ['id' => $familiar->id]) }}" class="px-5 py-2.5 text-sm font-semibold text-slate-600 bg-white border border-slate-300 rounded-xl hover:bg-slate-50 transition duration-150">
                            Cancelar
                        </a>
