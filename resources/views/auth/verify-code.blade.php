        ...
        <form method="GET" action="{{ route('password.reset', ['token' => 'verified-code']) }}" class="space-y-6">
            ...
            <div>
                ...
                <input 
                    ...
                    required 
                    autofocus 
                    placeholder="Campo para código" 
                    class="w-full px-4 py-3 bg-slate-50 border border-slate-300 rounded-xl text-slate-900 placeholder-slate-400 text-sm transition duration-200 focus:bg-white focus:outline-none focus:border-[#4EBA87] focus:ring-4 focus:ring-[#4EBA87]/15 @error('code') border-red-500 @enderror"
                />
                <x-input-error :messages="$errors->get('code')" class="mt-2 text-xs text-red-500" />
            </div>

            <!-- Submit Button (Verificar código) -->
            <div>
                <button 
                    type="submit" 
